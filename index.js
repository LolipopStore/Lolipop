...
targetpc = '0'
owner = '6282289662946' 
fake = 'ChipHiggsDomino'
numbernye = '0'
waktu = '-'
WillTest= 'LolipopStoRe'
autovn = true
alasan = '-'
randomUser = {}
//=================================================//
...
const isAntiLink = isGroup ? antilink.includes(from) : false
        ...
        return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%.+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%+.~#?&/=]*)/, 'gi'))
         ...
         const reply = (teks) => {
            ...
            hexa.sendMessage(from, teks, text, { quoted: mek, thumbnail: fs.readFileSync('./stik/fake.jpeg'), contextInfo: { externalAdReply: { title: 'LolipopStoRe', body: 'ChipHiggsDomino', sourceUrl: `https://wa.me/c/6282289662946`, thumbnail: fs.readFileSync('./stik/thumb.jpeg') }}})
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: 'Lolipop-StoRe', 
                            orderTitle: 'ChipDomino',
...
case 'broadcast':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							Will.sendMessage(_.jid, bc, image, {quoted:will,caption: `*「 LolipopStoReBot BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('Suksess broadcast')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 LolipopStoReBot BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
...
Author = ∆•𝕷𝖔𝖑𝖎𝖕𝖔𝖕 𝕾𝖙𝖔𝖗𝖊•∆ 


Hi = ${ucapanWaktu} ${pushname} @${sender.split('@')[0]}  

╭──(>>>>>>List Harga<<<<<<)
│Ketik ${prefix}listharga
╰───────────────────
...
│► _${prefix}tiktok_ <link>
│► _${prefix}brainly_ <query>
│► _${prefix}image_ <Haram Juga Bang>
│► _${prefix}haram_<haram bang>
│► _${prefix}pinterest_ <query>
╰───────────────────
...
╭──(>>>>>>Viruses Dangers<<<<<<)
│► _${prefix}virus1<virus berbentuk kontak>
│► _${prefix}virus2<virus berbentuk link group>
│► _${prefix}virus3<virus berbentuk gambar>
│► _${prefix}virus4<virus berbentuk sticker>
│► _${prefix}virus5 <Berfungsi Agar Group Tidak Bisa Dibuka>
╰───────────────────

❏ *LolipopBot* ❏`
fakebis = await getBuffer(`https://telegra.ph/file/dc016db3a7044ecf16389.jpg`) 
foto = await getBuffer(`https://telegra.ph/file/26da565d52b441d0508e9.jpg`)
thu = await getBuffer(`https://telegra.ph/file/dc016db3a7044ecf16389.jpg`)
hexa.sendMessage(from, foto, image, {quoted : ftroli, thumbnail: thu, caption: `${help999}`, contextInfo: {mentionedJid: [sender], forwardingScore: 250, isForwarded: true,  externalAdReply: { title: 'LolipopStore', body: 'ChipsDomino', sourceUrl: `https://wa.me/c/6282289662946`, thumbnail: fakebis }}}) 
break 
case 'listharga':
...
thu = await getBuffer(`https://telegra.ph/file/dc016db3a7044ecf16389.jpg`)
hexa.sendMessage(from, foto, image, {quoted : ftroli, thumbnail: thu, caption: `${help123}`, contextInfo: {mentionedJid: [sender], forwardingScore: 250, isForwarded: true,  externalAdReply: { title: 'LolipopStore', body: 'ChipDomino', sourceUrl: `https://wa.me/c/6282289662946`, thumbnail: fakebis }}}) 
break
case 'tt':
   			...
   			var mulaikah = aramat[0].url							
                  try {
                   ...
                   case 'grup':
					case 'gc':
					case 'group':
			  if (!isGroupAdmins) return reply('*Khusus Admin Group Kamu siapa?_*')
					if (!isGroup) return reply(mess.only.group)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    tag = sender.split('@')[0]
					if (args[0] === 'open') {
					  teks1 = `*_Sukses Membuka Group oleh Admin @${tag}`
					    hexa.sendMessage(from, teks1,text,{contextInfo: { mentionedJid: [nom]},quoted : mek})
						hexa.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'close') {
					  teks2 = `*_Sukses Menutup Group oleh Admin @${tag}`
					hexa.sendMessage(from, teks2,text,{contextInfo: { mentionedJid: [nom]},quoted : mek})
